...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:9px;">
  <div>
    ...
    <table style="font-size:0.7rem;border-collapse:collapse;"><tr>
      <td style="color:#888;padding-right:6px;">報告書番号</td>
      <td style="font-weight:700;padding-right:14px;">${doc.reportNumber||""}</td>
      <td style="color:#888;padding-right:6px;">作成日</td>
      <td style="font-weight:700;color:${accent};">${(doc.createdAt||"").replace(/-/g,"/")}</td>
    </tr></table>
  </div>
  <div style="text-align:right;font-size:0.68rem;line-height:1.8;color:#444;">
    <div style="font-weight:700;font-size:0.85rem;color:#1a1208;margin-bottom:2px;">${company.name}</div>
...
function EditPanel({ form, setField, setForm, setItem, clearItem, companies, staff, subjects, invoiceProducts, estimateProducts, docs, saveDoc, printPDF, setReportRow, clearReportRow, referralItems, reportClients, reportSubjects, onImportExcel }) {
  ...
  const { taxableSubtotal, nonTaxableSubtotal, discountAmt, tax, total, partialPayment, remaining } = isReport ? { taxableSubtotal:0,nonTaxableSubtotal:0,discountAmt:0,tax:0,total:0,partialPayment:0,remaining:0 } : calcTotals(form);
  const company  = companies.find((c) => c.id === form.companyId) || companies[0] || {};
  ...
  return (
    <div>
      ...
      <div style={{ ...S.card, padding: "1rem 1.5rem", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1 }} />
        <label title="XLS/Excelファイルから明細を自動入力" style={{ background: "#2d6a9e", color: "#fff", border: "none", borderRadius: 4, padding: "8px 16px", cursor: "pointer", fontFamily: "inherit", fontSize: "0.88rem", fontWeight: 700, letterSpacing: "0.05em", display: "flex", alignItems: "center", gap: 6 }}>
          📂 Excelから読み込む
...
// ============================================================
function MasterPanel({ companies, setCompanies, invoiceProducts, setInvoiceProducts, estimateProducts, setEstimateProducts, staff, setStaff, subjects, setSubjects, referralItems, setReferralItems, reportClients, setReportClients, reportSubjects, setReportSubjects }) {
        ...
        <div>
          ...
          {invSubTab === "subjects" && (
            <div><div style={S.sectionTitle}>件名マスター（請求書用）</div>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.85rem" }}>
                <thead><tr><th style={S.th}>件名</th><th style={{ ...S.th, width:140 }}></th></tr></thead>
                <tbody>{subjects.map((s) => (
                  <tr key={s.id}>{editSubj===s.id ? (<><td style={S.td}><input style={S.input} value={s.text} onChange={(e)=>setSubjects(p=>p.map(x=>x.id===s.id?{...x,text:e.target.value}:x))}/></td><td style={S.td}><button style={S.btnSm()} onClick={()=>setEditSubj(null)}>完了</button></td></>) : (<><td style={S.td}>{s.text}</td><td style={S.td}><button style={{...S.btnSm(),marginRight:6}} onClick={()=>setEditSubj(s.id)}>編集</button><button style={S.btnSm("#c0392b")} onClick={()=>setSubjects(p=>p.filter(x=>x.id!==s.id))}>削除</button></td></>)}</tr>
                ))}</tbody>
              </table>
              <div style={{marginTop:10}}><button style={S.btn()} onClick={()=>setSubjects(p=>[...p,{id:Date.now(),text:"新しい件名"}])}>＋ 件名追加</button></div>
            </div>
          )}
          {invSubTab === "staff" && (
            <div><div style={S.sectionTitle}>作成者マスター（共通）</div>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.85rem" }}>
                ...
                <tbody>{staff.map((s) => (
                  <tr key={s.id}>{editSt===s.id ? (<><td style={S.td}><input style={S.input} value={s.name} onChange={(e)=>setStaff(p=>p.map(x=>x.id===s.id?{...x,name:e.target.value}:x))}/></td><td style={S.td}><button style={S.btnSm()} onClick={()=>setEditSt(null)}>完了</button></td></>) : (<><td style={S.td}>{s.name}</td><td style={S.td}><button style={{...S.btnSm(),marginRight:6}} onClick={()=>setEditSt(s.id)}>編集</button><button style={S.btnSm("#c0392b")} onClick={()=>setStaff(p=>p.filter(x=>x.id!==s.id))}>削除</button></td></>)}</tr>
                ))}</tbody>
              </table>
              <div style={{marginTop:10}}><button style={S.btn()} onClick={()=>setStaff(p=>[...p,{id:Date.now(),name:"新担当者"}])}>＋ 担当者追加</button></div>
            </div>
          )}
        </div>
      )}
        ...
        <div>
          ...
          {rptSubTab === "subjects" && (
            <div><div style={S.sectionTitle}>件名マスター（報告書用）</div>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.85rem" }}>
                <thead><tr><th style={S.th}>件名</th><th style={{...S.th,width:140}}></th></tr></thead>
                <tbody>{reportSubjects.map((r) => (
                  <tr key={r.id}>{editRS===r.id ? (<><td style={S.td}><input style={S.input} value={r.text} onChange={(e)=>setReportSubjects(p=>p.map(x=>x.id===r.id?{...x,text:e.target.value}:x))}/></td><td style={S.td}><button style={S.btnSm()} onClick={()=>setEditRS(null)}>完了</button></td></>) : (<><td style={S.td}>{r.text}</td><td style={S.td}><button style={{...S.btnSm(),marginRight:6}} onClick={()=>setEditRS(r.id)}>編集</button><button style={S.btnSm("#c0392b")} onClick={()=>setReportSubjects(p=>p.filter(x=>x.id!==r.id))}>削除</button></td></>)}</tr>
                ))}</tbody>
              </table>
              <div style={{marginTop:10}}><button style={S.btn()} onClick={()=>setReportSubjects(p=>[...p,{id:Date.now(),text:"新しい件名"}])}>＋ 件名追加</button></div>
            </div>
          )}
          {rptSubTab === "referrals" && (
            <div><div style={S.sectionTitle}>ご紹介内容マスター（報告書用）</div>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.85rem" }}>
                <thead><tr><th style={S.th}>ご紹介内容</th><th style={{...S.th,width:140}}></th></tr></thead>
                <tbody>{referralItems.map((r) => (
                  <tr key={r.id}>{editRef===r.id ? (<><td style={S.td}><input style={S.input} value={r.text} onChange={(e)=>setReferralItems(p=>p.map(x=>x.id===r.id?{...x,text:e.target.value}:x))}/></td><td style={S.td}><button style={S.btnSm()} onClick={()=>setEditRef(null)}>完了</button></td></>) : (<><td style={S.td}>{r.text}</td><td style={S.td}><button style={{...S.btnSm(),marginRight:6}} onClick={()=>setEditRef(r.id)}>編集</button><button style={S.btnSm("#c0392b")} onClick={()=>setReferralItems(p=>p.filter(x=>x.id!==r.id))}>削除</button></td></>)}</tr>
                ))}</tbody>
              </table>
              <div style={{marginTop:10}}><button style={S.btn()} onClick={()=>setReferralItems(p=>[...p,{id:Date.now(),text:"新しいご紹介内容"}])}>＋ ご紹介内容追加</button></div>
            </div>
          )}
          {rptSubTab === "staff" && (
            <div><div style={S.sectionTitle}>作成者マスター（共通）</div>
...
export default function App() {
  ...
  useEffect(() => { persist(KEYS.referrals,    referralItems);    if (fbDb) fbSetMaster(fbDb, "referralItems",    referralItems);    }, [referralItems,    fbDb]);
  useEffect(() => { persist(KEYS.rptClients,   reportClients);    if (fbDb) fbSetMaster(fbDb, "reportClients",    reportClients);    }, [reportClients,   fbDb]);
  useEffect(() => { persist(KEYS.rptSubjects,  reportSubjects);   if (fbDb) fbSetMaster(fbDb, "reportSubjects",   reportSubjects);   }, [reportSubjects,  fbDb]);
  ...
  const handleImportExcel = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        const wb   = XLSXLib.read(data, { type: "uint8array" });
        ...
        rows.forEach((row, i) => {
          ...
          // 宛先行（col6が電話番号形式、col7が氏名）
          if (col6 && /^\d{4}-\d{4}-\d{4}/.test(col6) && col7) {
          ...
          if (col5 === "売上" && col7 && !col7.includes("※") && !col7.includes("計")) {
            ...
            const qty = Number(row[11]) || 1;
            const amt = Number(row[13]) || 0;
            ...
            items.push({ name: col7.trim(), qty, price, taxable: true });
          }
        });
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 48, borderBottom: "1px solid #c8a96e22" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", lineHeight: 1 }}>
              ...
              <span style={{ fontSize: "0.62rem", color: "#c8a96e88", letterSpacing: "0.1em", marginTop: 1 }}>Ver1-001</span>
            </div>
            ...
            <div style={{ padding: 24 }}>
              ...
              <div style={{ background: "#f5f0eb", borderRadius: 6, padding: "12px 16px", marginBottom: 16, fontSize: "0.85rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  {[["宛先", importPreview.clientName || "（未取得）"],["担当者", importPreview.staff],["施行日", importPreview.workDate || "（未取得）"],["式場", importPreview.venueName || "（未取得）"],["件名", importPreview.subject]].map(([l,v]) => (
                    <div key={l} style={{ display: "flex", gap: 8 }}>
                ...
                <tbody>
                  {importPreview.items.filter(i => i.name).map((item, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? "#fffdf9" : "#f7f3ee" }}>
                      ...
                      <td style={{ padding: "5px 10px", borderBottom: "1px solid #e0d8cc", textAlign: "center" }}>{item.qty}</td>
                      <td style={{ padding: "5px 10px", borderBottom: "1px solid #e0d8cc", textAlign: "right" }}>¥ {fmt(item.price)}</td>
                      <td style={{ padding: "5px 10px", borderBottom: "1px solid #e0d8cc", textAlign: "right", fontWeight: 600 }}>¥ {fmt(item.qty * item.price)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
