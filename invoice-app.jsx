...
function buildPrintHTML(doc, company) {
  ...
  const itemRows = allItems.slice(0, 10).map((item, i) => {
    ...
    if (isEmpty) {
      amtCell = "";
    } else if (isEst && itemDiscount > 0) {
      const label = item.discountType === "%" ? `${item.discount}%引` : "値引";
      amtCell = `<span style="text-decoration:line-through;color:#999;font-size:0.78em;">¥ ${fmt(gross)}</span><br><span style="color:#c0392b;font-size:0.72em;">${label} －¥ ${fmt(itemDiscount)}</span><br>¥ ${fmt(net)}`;
    } else {
      amtCell = `¥ ${fmt(gross)}`;
    }
    ...
    return `<tr style="background:${bg};">
      ...
      <td style="padding:7px 10px;border-bottom:1px solid #ccc;text-align:right;font-weight:${isEmpty ? 400 : 700};color:#111;">${amtCell}</td>
    </tr>`;
  }).join("");
