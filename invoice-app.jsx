...
const EMPTY_ITEM  = () => ({ name: "", qty: 1, price: 0, taxable: true });
...
const KEYS = {
  ...
  staff: "inv_staff4", subjects: "inv_subjects4", trash: "inv_trash4",
};
...
function calcTotals(form) {
  ...
  for (const i of form.items) {
    ...
    if (i.taxable !== false) taxableSubtotal += amt;
    else nonTaxableSubtotal += amt;
  }
  const subtotal = taxableSubtotal + nonTaxableSubtotal;

  // 割引は課税小計に対してのみ適用（非課税品は割引対象外）
  const discountAmt = form.discountType === "%"
    ? Math.round(taxableSubtotal * (Number(form.discount || 0) / 100))
    : Math.min(Number(form.discount || 0), taxableSubtotal);

  const taxableAfterDiscount = taxableSubtotal - discountAmt;
  const tax   = Math.round(taxableAfterDiscount * 0.1);
  const total = taxableAfterDiscount + nonTaxableSubtotal + tax;

  // 一部入金差引（請求書のみ）
  const partialPayment = Number(form.partialPayment || 0);
  const remaining      = total - partialPayment;

  return { taxableSubtotal, nonTaxableSubtotal, subtotal, discountAmt, taxableAfterDiscount, tax, total, partialPayment, remaining };
}
  ...
  for (let i = 0; i < Math.max(pi.length, ni.length); i++) {
    ...
    if (p.name !== n.name || String(p.qty) !== String(n.qty) || String(p.price) !== String(n.price) || p.taxable !== n.taxable) {
      ...
      if (!p.name && n.name) changes.push(`明細${row}行目：追加「${n.name}」`);
        ...
        if (p.taxable !== n.taxable) diffs.push(`課税区分${p.taxable !== false ? "課税" : "非課税"}→${n.taxable !== false ? "課税" : "非課税"}`);
...
function buildPrintHTML(doc, company) {
  const { taxableSubtotal, nonTaxableSubtotal, discountAmt, tax, total, partialPayment, remaining } = calcTotals(doc);
  ...
  const itemRows = allItems.slice(0, 10).map((item, i) => {
    ...
    const amt     = isEmpty ? "" : `¥ ${fmt(Number(item.qty) * Number(item.price))}`;
    const taxBadge = isEmpty ? "" : (item.taxable !== false
      ? `<span style="font-size:0.6rem;background:#e8f4e8;color:#2d7a2d;border:1px solid #b0d8b0;border-radius:2px;padding:1px 4px;margin-left:4px;">課税</span>`
      : `<span style="font-size:0.6rem;background:#f4f4e8;color:#7a7a2d;border:1px solid #d8d8b0;border-radius:2px;padding:1px 4px;margin-left:4px;">非課税</span>`);
    const bg = i % 2 === 0 ? "#fffdf9" : "#f7f3ee";
    return `<tr style="background:${bg};">
      <td style="padding:6px 10px;border-bottom:1px solid #e0d8cc;height:28px;">${item.name || ""}${taxBadge}</td>
      <td style="padding:6px 10px;border-bottom:1px solid #e0d8cc;text-align:center;">${isEmpty ? "" : item.qty}</td>
      <td style="padding:6px 10px;border-bottom:1px solid #e0d8cc;text-align:right;">${isEmpty ? "" : "¥ " + fmt(item.price)}</td>
      <td style="padding:6px 10px;border-bottom:1px solid #e0d8cc;text-align:right;font-weight:${isEmpty ? 400 : 600};">${amt}</td>
    </tr>`;
  }).join("");
  ...
  // 合計行ブロック
  const summaryRows = `
    <tr><td style="padding:4px 20px 4px 10px;color:#666;border-bottom:1px solid #e0d8cc;">課税小計</td><td style="padding:4px 10px;text-align:right;border-bottom:1px solid #e0d8cc;min-width:115px;">¥ ${fmt(taxableSubtotal)}</td></tr>
    ${nonTaxableSubtotal > 0 ? `<tr><td style="padding:4px 20px 4px 10px;color:#666;border-bottom:1px solid #e0d8cc;">非課税小計</td><td style="padding:4px 10px;text-align:right;border-bottom:1px solid #e0d8cc;">¥ ${fmt(nonTaxableSubtotal)}</td></tr>` : ""}
    <tr><td style="padding:4px 20px 4px 10px;color:#c0392b;border-bottom:1px solid #e0d8cc;">${doc.discountType === "%" ? `割引（${doc.discount}%）` : "値引"}</td><td style="padding:4px 10px;text-align:right;border-bottom:1px solid #e0d8cc;color:#c0392b;">${discountAmt > 0 ? `－¥ ${fmt(discountAmt)}` : "—"}</td></tr>
    <tr><td style="padding:4px 20px 4px 10px;color:#666;border-bottom:1px solid #e0d8cc;">消費税（10%）</td><td style="padding:4px 10px;text-align:right;border-bottom:1px solid #e0d8cc;">¥ ${fmt(tax)}</td></tr>
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:9px;">
  <div>
    ...
    <table style="font-size:0.7rem;border-collapse:collapse;"><tr>
      <td style="color:#888;padding-right:6px;">${isInv ? "請求書番号" : "見積書番号"}</td>
      <td style="font-weight:700;padding-right:14px;">${doc.docNumber}</td>
      <td style="color:#888;padding-right:6px;">作成日</td>
      <td style="font-weight:700;padding-right:14px;">${doc.createdAt}</td>
      <td style="color:#888;padding-right:6px;">${isInv ? "支払期限" : "有効期限"}</td>
      <td style="font-weight:700;color:${isInv ? "#c0392b" : accent};padding-right:14px;">${doc.dueDate}</td>
      <td style="color:#888;padding-right:6px;">施行日</td>
      <td style="font-weight:700;color:${accent};">${doc.workDate || "—"}</td>
    </tr></table>
  </div>
  <div style="text-align:right;font-size:0.68rem;line-height:1.8;color:#444;">
    ...
    ${company.invoiceNo ? `<div style="font-size:0.65rem;color:#888;margin-top:1px;">登録番号: ${company.invoiceNo}</div>` : ""}
  </div>
</div>
<div style="height:2px;background:linear-gradient(to right,${accent},${gold},#f5f0eb);margin-bottom:9px;"></div>
<div style="margin-bottom:9px;">
  <div style="font-size:0.98rem;font-weight:700;border-bottom:2px solid ${gold};padding-bottom:3px;margin-bottom:5px;display:inline-block;">${doc.clientName || "　"}　御中</div>
  ...
  <div style="font-size:0.72rem;color:#666;display:flex;gap:8px;margin-top:2px;"><span style="white-space:nowrap;">備考</span><span>${doc.note || ""}</span></div>
</div>
<table style="width:100%;border-collapse:collapse;font-size:0.77rem;margin-bottom:8px;">
  <thead><tr>
    <th style="background:${accent};color:${gold};padding:6px 10px;text-align:left;width:52%;font-weight:600;">品目</th>
    <th style="background:${accent};color:${gold};padding:6px 10px;text-align:center;width:12%;font-weight:600;">数量</th>
    <th style="background:${accent};color:${gold};padding:6px 10px;text-align:right;width:18%;font-weight:600;">単価（円）</th>
    <th style="background:${accent};color:${gold};padding:6px 10px;text-align:right;width:18%;font-weight:600;">金額（円）</th>
  </tr></thead>
...
${isInv ? `<div style="border:1px solid ${gold};border-radius:2px;padding:7px 14px;font-size:0.73rem;"><span style="font-weight:700;color:${accent};margin-right:12px;">■ お振込先</span><span style="color:#444;">${company.bank}</span></div>` : ""}
</body></html>`;
}
...
const S = {
  ...
  taxBadge: (taxable) => ({ display: "inline-block", fontSize: "0.68rem", padding: "1px 6px", borderRadius: 3, fontWeight: 700, background: taxable !== false ? "#e8f4e8" : "#f4f4e8", color: taxable !== false ? "#2d7a2d" : "#7a7a2d", border: `1px solid ${taxable !== false ? "#b0d8b0" : "#d8d8b0"}`, marginLeft: 4 }),
...
function ItemRow({ item, idx, products, onChange, onClear }) {
  return (
    <tr style={{ background: idx % 2 === 0 ? "#fffdf9" : "#f7f3ee" }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <select style={{ ...S.select, flex: "0 0 auto", width: 148, fontSize: "0.78rem" }} value=""
            onChange={(e) => {
              ...
              if (p) onChange(idx, { name: p.name, price: p.price, taxable: p.taxable !== false });
            }}>
            <option value="">マスターから選択</option>
            {products.map((p) => <option key={p.id} value={p.name}>{p.name}（{p.taxable !== false ? "課税" : "非課税"}）</option>)}
          </select>
      ...
      {/* 課税区分 */}
      <td style={{ ...S.td, width: "9%", textAlign: "center" }}>
        <select style={{ ...S.select, fontSize: "0.78rem", padding: "5px 4px", textAlign: "center" }}
          value={item.taxable !== false ? "taxable" : "non"}
          onChange={(e) => onChange(idx, { taxable: e.target.value === "taxable" })}>
      ...
      {/* 金額 */}
      <td style={{ ...S.td, width: "14%", textAlign: "right", fontWeight: 600 }}>
        {item.name ? `¥ ${fmt(Number(item.qty) * Number(item.price))}` : "—"}
      </td>
      {/* クリア */}
      ...
      <div style={{ background: "#fff", borderRadius: 10, width: "100%", maxWidth: 760, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 48px #0005", fontFamily: "inherit" }}>
        ...
        <div style={{ padding: "24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14, fontSize: "0.82rem" }}>
            {[["作成日", doc.createdAt], [isInv ? "支払期限" : "有効期限", doc.dueDate], ["施行日", doc.workDate || "—"], ["担当者", doc.staff], ["宛先", doc.clientName], ["件名", doc.subject]].map(([l, v]) => (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem", marginBottom: 12 }}>
            <thead><tr>
              {[["品目","46%","left"],["課税","9%","center"],["数量","10%","center"],["単価","17%","right"],["金額","18%","right"]].map(([h,w,a]) => (
            ...
            <tbody>
              {filledItems.map((item, i) => (
                <tr key={i} style={{ background: i % 2 === 0 ? "#fffdf9" : "#f7f3ee" }}>
                  ...
                  <td style={{ padding: "5px 10px", borderBottom: "1px solid #e0d8cc", textAlign: "center" }}>
                    <span style={S.taxBadge(item.taxable)}>{item.taxable !== false ? "課税" : "非課税"}</span>
                  </td>
                  ...
                  <td style={{ padding: "5px 10px", borderBottom: "1px solid #e0d8cc", textAlign: "right", fontWeight: 600 }}>¥ {fmt(Number(item.qty) * Number(item.price))}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
            <table style={{ fontSize: "0.82rem", borderCollapse: "collapse", minWidth: 280 }}>
              <tbody>
                <tr><td style={{ padding: "4px 20px 4px 10px", color: "#666", borderBottom: "1px solid #e0d8cc" }}>課税小計</td><td style={{ padding: "4px 10px", textAlign: "right", borderBottom: "1px solid #e0d8cc", minWidth: 110 }}>¥ {fmt(taxableSubtotal)}</td></tr>
                {nonTaxableSubtotal > 0 && <tr><td style={{ padding: "4px 20px 4px 10px", color: "#666", borderBottom: "1px solid #e0d8cc" }}>非課税小計</td><td style={{ padding: "4px 10px", textAlign: "right", borderBottom: "1px solid #e0d8cc" }}>¥ {fmt(nonTaxableSubtotal)}</td></tr>}
                <tr><td style={{ padding: "4px 20px 4px 10px", color: "#c0392b", borderBottom: "1px solid #e0d8cc" }}>{doc.discountType === "%" ? `割引（${doc.discount}%）` : "値引"}</td><td style={{ padding: "4px 10px", textAlign: "right", borderBottom: "1px solid #e0d8cc", color: "#c0392b" }}>{discountAmt > 0 ? `－¥ ${fmt(discountAmt)}` : "—"}</td></tr>
                <tr><td style={{ padding: "4px 20px 4px 10px", color: "#666", borderBottom: "1px solid #e0d8cc" }}>消費税（10%）</td><td style={{ padding: "4px 10px", textAlign: "right", borderBottom: "1px solid #e0d8cc" }}>¥ {fmt(tax)}</td></tr>
...
// EDIT PANEL
// ============================================================
function EditPanel({ form, setField, setForm, setItem, clearItem, companies, staff, subjects, invoiceProducts, estimateProducts, docs, saveDoc, printPDF }) {
  const products = form.type === "invoice" ? invoiceProducts : estimateProducts;
  const { taxableSubtotal, nonTaxableSubtotal, discountAmt, tax, total, partialPayment, remaining } = calcTotals(form);
  ...
  return (
    <div>
      {/* 操作バー */}
      <div style={{ ...S.card, padding: "1rem 1.5rem", display: "flex", alignItems: "center", gap: 16 }}>
        <span style={{ fontWeight: 700, color: "#7a5c2e", fontSize: "0.88rem" }}>書類種別:</span>
        {[["invoice","請求書"],["estimate","見積書"]].map(([v,l]) => (
          <label key={v} style={{ display: "flex", alignItems: "center", gap: 5, cursor: "pointer", fontWeight: form.type === v ? 700 : 400, fontSize: "0.88rem" }}>
            <input type="radio" checked={form.type === v} onChange={() => setForm((f) => ({ ...f, type: v, docNumber: formatDocNumber(v, docs) }))} /> {l}
          </label>
        ))}
        <div style={{ flex: 1 }} />
      ...
      <div style={S.card}>
        ...
        <div style={{ marginBottom: 10 }}>
          <label style={S.label}>宛先（会社名）</label>
          <input style={S.input} placeholder="株式会社〇〇" value={form.clientName} onChange={(e) => setField("clientName", e.target.value)} />
        </div>
        <div style={S.grid2}>
          <div>
      ...
      <div style={S.card}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>
            <th style={{ ...S.th, width: "46%" }}>品目</th>
            <th style={{ ...S.th, width: "9%", textAlign: "center" }}>課税区分</th>
      ...
      {/* 割引・入金・合計 */}
      <div style={S.card}>
        <div style={S.sectionTitle}>割引 / {isInv ? "入金・" : ""}合計</div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 24, flexWrap: "wrap" }}>
          ...
          <table style={{ fontSize: "0.88rem", borderCollapse: "collapse", minWidth: 290 }}>
            <tbody>
              <tr>
                <td style={{ padding: "4px 20px 4px 8px", color: "#666" }}>課税小計</td>
                <td style={{ padding: "4px 8px", textAlign: "right", minWidth: 115 }}>¥ {fmt(taxableSubtotal)}</td>
              </tr>
              {nonTaxableSubtotal > 0 && (
                <tr>
                  <td style={{ padding: "4px 20px 4px 8px", color: "#666" }}>非課税小計</td>
                  <td style={{ padding: "4px 8px", textAlign: "right" }}>¥ {fmt(nonTaxableSubtotal)}</td>
                </tr>
              )}
              <tr>
                <td style={{ padding: "4px 20px 4px 8px", color: "#c0392b" }}>
                  {form.discountType === "%" ? `割引（${form.discount}%）` : "値引"}
                </td>
                <td style={{ padding: "4px 8px", textAlign: "right", color: "#c0392b" }}>
                  {discountAmt > 0 ? `－¥ ${fmt(discountAmt)}` : "—"}
                </td>
              </tr>
              <tr>
                <td style={{ padding: "4px 20px 4px 8px", color: "#666" }}>消費税（10%）</td>
                <td style={{ padding: "4px 8px", textAlign: "right" }}>¥ {fmt(tax)}</td>
              </tr>
...
function HistoryPanel({ docs, calcTotals, onView, onEdit, onTrash }) {
  ...
  const months      = [...new Set(docs.map((d) => ym(d.createdAt)).filter(Boolean))].sort().reverse();
  const workYMs     = [...new Set(docs.map((d) => ym(d.workDate)).filter(Boolean))].sort().reverse();
  const staffList   = [...new Set(docs.map((d) => d.staff).filter(Boolean))];
  const subjectList = [...new Set(docs.map((d) => d.subject).filter(Boolean))].sort();
  ...
  return (
    <div>
      <div style={S.card}>
        <div style={S.sectionTitle}>フィルター</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: "1rem" }}>
          <div><label style={S.label}>種別</label>
            <select style={S.select} value={fType} onChange={(e) => setFType(e.target.value)}>
              <option value="all">すべて</option><option value="invoice">請求書</option><option value="estimate">見積書</option>
            </select>
          </div>
          <div><label style={S.label}>作成年月</label>
          ...
          <div><label style={S.label}>施行年月</label>
          ...
          <div><label style={S.label}>件名</label>
        ...
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 12 }}>
          <button style={S.btnSm("#888")} onClick={() => { setFType("all"); setFMonth(""); setFStaff(""); setFWorkYM(""); setFSubject(""); }}>リセット</button>
          <span style={{ fontSize: "0.8rem", color: "#999" }}>{filtered.length} 件表示</span>
        </div>
      </div>
      <div style={S.card}>
        <div style={S.sectionTitle}>発行履歴（新しい順）</div>
        {filtered.length === 0 && <p style={{ color: "#aaa", textAlign: "center", padding: "2rem 0" }}>{docs.length === 0 ? "まだ保存された書類はありません。" : "条件に一致する書類がありません。"}</p>}
        {filtered.length > 0 && (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
            <thead><tr>
              <th style={S.th}>種別</th><th style={S.th}>番号</th><th style={S.th}>作成日</th><th style={S.th}>施行日</th>
              <th style={S.th}>担当者</th><th style={S.th}>宛先</th><th style={S.th}>件名</th>
              <th style={{ ...S.th, textAlign: "right" }}>合計</th>
              <th style={{ ...S.th, textAlign: "center", width: 164 }}>操作</th>
            </tr></thead>
            <tbody>
              {filtered.map((d, i) => {
                ...
                return (
                  <tr key={d.id} style={{ background: i % 2 === 0 ? "#fffdf9" : "#f7f3ee" }}>
                    ...
                    <td style={{ ...S.td, whiteSpace: "nowrap" }}>{d.docNumber}</td>
                    <td style={{ ...S.td, whiteSpace: "nowrap" }}>{d.createdAt}</td>
                    <td style={{ ...S.td, whiteSpace: "nowrap", color: "#7a5c2e" }}>{d.workDate || "—"}</td>
                    <td style={S.td}>{d.staff}</td>
                    <td style={S.td}>{d.clientName}</td>
                    <td style={S.td}>{d.subject}</td>
                    <td style={{ ...S.td, textAlign: "right", fontWeight: 600, whiteSpace: "nowrap" }}>¥ {fmt(total)}</td>
                    <td style={{ ...S.td, textAlign: "center", whiteSpace: "nowrap" }}>
                      ...
                      <button style={S.btnSm("#c0392b")} onClick={() => onTrash(d.id)}>🗑 ゴミ箱</button>
                    </td>
                  </tr>
                );
              })}
    ...
    <div>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
        <thead><tr>
          ...
          <th style={{ ...S.th, width: 90, textAlign: "center" }}>課税区分</th>
              ...
              >
                ...
                {editId === p.id ? (
                  <>
                    ...
                    <td style={S.td}>
                      <select style={S.select} value={p.taxable !== false ? "taxable" : "non"}
                        onChange={(e) => setProducts((prev) => prev.map((x) => x.id === p.id ? { ...x, taxable: e.target.value === "taxable" } : x))}>
                        <option value="taxable">課税</option>
                        <option value="non">非課税</option>
                      </select>
                    </td>
                  ...
                  <>
                    ...
                    <td style={{ ...S.td, textAlign: "center" }}><span style={S.taxBadge(p.taxable)}>{p.taxable !== false ? "課税" : "非課税"}</span></td>
      ...
      <div style={{ marginTop: 10 }}>
        <button style={S.btn()} onClick={() => setProducts((p) => [...p, { id: Date.now(), name: "新商品", price: 0, taxable: true }])}>＋ 商品追加</button>
      </div>
    </div>
  );
}
...
// ============================================================
function MasterPanel({ companies, setCompanies, invoiceProducts, setInvoiceProducts, estimateProducts, setEstimateProducts, staff, setStaff, subjects, setSubjects }) {
  ...
  return (
    <div style={S.card}>
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {[["company","会社情報"],["invProducts","請求書用商品"],["estProducts","見積書用商品"],["staff","作成者"],["subjects","件名"]].map(([k,l]) => (
      ...
      {activeM === "company" && (
        <div>
          <div style={S.sectionTitle}>会社情報マスター</div>
          {companies.map((c) => (
      ...
      {activeM === "invProducts" && (
        <div>
          <div style={S.sectionTitle}>請求書用 商品マスター</div>
      ...
      {activeM === "estProducts" && (
        <div>
          <div style={S.sectionTitle}>見積書用 商品マスター</div>
      ...
      {activeM === "staff" && (
        <div>
          <div style={S.sectionTitle}>作成者マスター</div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
            <thead><tr><th style={S.th}>氏名</th><th style={{ ...S.th, width: 140 }}></th></tr></thead>
            <tbody>
              {staff.map((s) => (
                <tr key={s.id}>
                  {editSt === s.id ? (
                    <>
                      <td style={S.td}><input style={S.input} value={s.name} onChange={(e) => setStaff((prev) => prev.map((x) => x.id === s.id ? { ...x, name: e.target.value } : x))} /></td>
                      <td style={S.td}><button style={S.btnSm()} onClick={() => setEditSt(null)}>完了</button></td>
                    </>
                  ) : (
                    <>
                      <td style={S.td}>{s.name}</td>
                      <td style={S.td}>
                        <button style={{ ...S.btnSm(), marginRight: 6 }} onClick={() => setEditSt(s.id)}>編集</button>
                        <button style={S.btnSm("#c0392b")} onClick={() => setStaff((prev) => prev.filter((x) => x.id !== s.id))}>削除</button>
                      </td>
                    </>
                  )}
                </tr>
          ...
          <div style={{ marginTop: 10 }}>
            <button style={S.btn()} onClick={() => setStaff((p) => [...p, { id: Date.now(), name: "新担当者" }])}>＋ 担当者追加</button>
          </div>
        </div>
      )}
      ...
      {activeM === "subjects" && (
        <div>
              ...
              {subjects.map((s) => (
                <tr key={s.id}>
                  {editSubj === s.id ? (
                    <>
                      <td style={S.td}>
                        <input style={S.input} value={s.text}
                          onChange={(e) => setSubjects((prev) => prev.map((x) => x.id === s.id ? { ...x, text: e.target.value } : x))} />
                      </td>
                      <td style={S.td}><button style={S.btnSm()} onClick={() => setEditSubj(null)}>完了</button></td>
                    </>
                  ) : (
                    <>
                      <td style={S.td}>{s.text}</td>
                      <td style={S.td}>
                        <button style={{ ...S.btnSm(), marginRight: 6 }} onClick={() => setEditSubj(s.id)}>編集</button>
                        <button style={S.btnSm("#c0392b")} onClick={() => setSubjects((prev) => prev.filter((x) => x.id !== s.id))}>削除</button>
                      </td>
                    </>
                  )}
                </tr>
          ...
          <div style={{ marginTop: 10 }}>
            <button style={S.btn()} onClick={() => setSubjects((p) => [...p, { id: Date.now(), text: "新しい件名" }])}>＋ 件名追加</button>
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  useEffect(() => { persist(KEYS.companies,   companies);        if (fbDb) fbSetMaster(fbDb, "companies",        companies);        }, [companies,        fbDb]);
  useEffect(() => { persist(KEYS.invProducts,  invoiceProducts);  if (fbDb) fbSetMaster(fbDb, "invoiceProducts",  invoiceProducts);  }, [invoiceProducts,  fbDb]);
  useEffect(() => { persist(KEYS.estProducts,  estimateProducts); if (fbDb) fbSetMaster(fbDb, "estimateProducts", estimateProducts); }, [estimateProducts, fbDb]);
  useEffect(() => { persist(KEYS.staff,        staff);            if (fbDb) fbSetMaster(fbDb, "staff",            staff);            }, [staff,            fbDb]);
  useEffect(() => { persist(KEYS.subjects,     subjects);         if (fbDb) fbSetMaster(fbDb, "subjects",         subjects);         }, [subjects,         fbDb]);
  ...
  const clearItem = (idx) => setItem(idx, { name: "", qty: 1, price: 0, taxable: true });
  ...
  const printPDF = (doc) => {
    ...
    const html = buildPrintHTML(doc, co);
    setPrintHTML(html);
  };
  ...
  return (
    <div style={S.app}>
      ...
      <header style={{ ...S.header, height: "auto", flexDirection: "column", alignItems: "stretch", padding: "0 2rem" }}>
        {/* 1行目：タイトル ＋ ナビ */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 56 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <h1 style={S.headerTitle}>📄 請求書 / 見積書 発行システム</h1>
            <span style={{
              ...
              {fbStatus === "online" ? "🟢 クラウド同期中" : fbStatus === "connecting" ? "🟡 接続中..." : "🔴 オフライン（ローカル保存）"}
            </span>
          </div>
          <div style={S.row}>
          ...
          <button style={{ ...S.navBtn(tab === "trash"), color: tab === "trash" ? "#1a1208" : "#e8a0a0", borderColor: "#e8a0a066" }} onClick={() => setTab("trash")}>🗑 ゴミ箱{trash.length > 0 ? ` (${trash.length})` : ""}</button>
          </div>
        </div>
        {/* 2行目：バックアップ・リストア */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, height: 40, borderTop: "1px solid #c8a96e22" }}>
          <span style={{ fontSize: "0.72rem", color: "#c8a96e88", marginRight: 4 }}>データ管理:</span>
          <button
            title="全データをJSONファイルに保存"
            onClick={handleBackup}
            style={{ background: "#2d6a9e", color: "#fff", border: "none", borderRadius: 4, padding: "5px 14px", cursor: "pointer", fontFamily: "inherit", fontSize: "0.8rem", fontWeight: 700 }}>
            💾 バックアップ
          </button>
          <label
            title="バックアップJSONを読み込んで全データを復元"
            style={{ background: "#7a5c2e", color: "#fff", border: "none", borderRadius: 4, padding: "5px 14px", cursor: "pointer", fontFamily: "inherit", fontSize: "0.8rem", fontWeight: 700 }}>
            📂 リストア
