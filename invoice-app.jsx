...
// createdAt の有無や型に関わらず取りこぼさないよう、orderByは使わず
// 全件取得してからアプリ側で新しい順に並べ替える
...
async function fbGetDocs(db, collName) {
  ...
  return sortByCreatedDesc(snap.docs.map(d => ({ ...d.data(), _fbId: d.id })));
}
// Firestore は undefined を保存できないため、保存前に undefined を取り除く
// （null は保存可能なので残す。配列・ネストしたオブジェクトも再帰的に処理）
function stripUndefined(value) {
  ...
  if (value && typeof value === "object") {
    ...
    for (const k in value) {
      if (value[k] === undefined) continue; // undefined のキーは丸ごと除外
...
function fbSubscribe(db, collName, onChange, onError) {
  // orderBy を付けない（createdAt欠落ドキュメントの取りこぼしを防ぐ）。並べ替えはアプリ側で実施。
  return db.collection(collName)
    .onSnapshot(
      snap => {
        onChange(sortByCreatedDesc(snap.docs.map(d => ({ ...d.data(), _fbId: d.id }))));
      },
      err => {
        // 購読が失敗したら検知できるようにする（従来は握りつぶされていた）
        console.error("Firestore購読エラー(" + collName + "):", err);
...
// 書類ID：時刻(36進)＋長めの乱数で、別端末・別時刻での衝突を実質ゼロに
const genId        = () => Date.now().toString(36).toUpperCase() + "-" +
                           Math.random().toString(36).slice(2, 10).toUpperCase();
// 書類タイプ → 接頭辞
const docPrefix = (type) => type === "invoice" ? "INV" : type === "estimate" ? "EST" : type === "report_tax" ? "RPT" : "RPN";
// 本日の yyyymmdd
const ymd = () => {
  const d = new Date();
  return `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}${String(d.getDate()).padStart(2, "0")}`;
};
// 書類の番号文字列を取り出す（請求書/見積書は docNumber・報告書は reportNumber）
const getDocNo = (d) => d.docNumber || d.reportNumber || "";
// 新形式番号をパースする。形式: PREFIX-yyyymmdd-xxx-zz
// 返り値 { prefix, date, seq, copy } / 旧形式や不一致なら null
const parseDocNumber = (no) => {
  const m = /^([A-Z]+)-(\d{8})-(\d{3})-(\d{2})$/.exec(no || "");
  if (!m) return null;
  return { prefix: m[1], date: m[2], seq: Number(m[3]), copy: Number(m[4]) };
};
// 新規作成時の番号：本日・同タイプの新形式番号から「その日の作成順 xxx」を採番し、枝番は 01
const formatDocNumber = (type, docs) => {
  const prefix = docPrefix(type);
  const date   = ymd();
  // 本日・同タイプ・新形式の番号のうち最大の seq を探す（翌日は該当なし→0 から）
  let maxSeq = 0;
  docs.forEach((d) => {
    if (d.type !== type) return;
    const p = parseDocNumber(getDocNo(d));
    if (p && p.prefix === prefix && p.date === date) {
      if (p.seq > maxSeq) maxSeq = p.seq;
    }
  });
  const seq = String(maxSeq + 1).padStart(3, "0");
  return `${prefix}-${date}-${seq}-01`;
};
// コピー時の番号：元番号の PREFIX-yyyymmdd-xxx を引き継ぎ、枝番だけ +1（02, 03, ...）
const formatCopyNumber = (srcNo, docs) => {
  const p = parseDocNumber(srcNo);
  if (!p) return null; // 新形式でなければコピー番号は作れない
  const base = `${p.prefix}-${p.date}-${String(p.seq).padStart(3, "0")}`;
  let maxCopy = 1;
  docs.forEach((d) => {
    const q = parseDocNumber(getDocNo(d));
    if (q && q.prefix === p.prefix && q.date === p.date && q.seq === p.seq) {
      if (q.copy > maxCopy) maxCopy = q.copy;
    }
  });
  return `${base}-${String(maxCopy + 1).padStart(2, "0")}`;
};
// 枝番が 01（オリジナル）かどうか
const isOriginalCopy = (no) => {
  const p = parseDocNumber(no);
  return p ? p.copy === 1 : false;
};
...
function calcTotals(form) {
  ...
  const items = form.items || []; // 報告書など items を持たない書類でも落ちないように
  // 値引前の小計（各明細の値引前金額の合計）
  ...
  if (isEstimate) {
    // 見積書：商品ごとの値引きを合算
    discountAmt = items.reduce((s, i) => s + itemAmounts(i).itemDiscount, 0);
  } else {
    // 請求書：従来どおり全体の割引・値引き
  ...
  const itemRows = allItems.slice(0, 10).map((item, i) => {
    ...
    if (isEst) {
      ...
      const discCell = (!isEmpty && itemDiscount !== 0)
        ? `<span style="color:#c0392b;font-size:0.82em;">${item.discountType === "%" ? `${item.discount}%引 ` : "値引 "}${itemDiscount > 0 ? "－" : "＋"}¥ ${fmt(Math.abs(itemDiscount))}</span>`
...
function DetailModal({ doc, company, onClose, onEdit, onPrint, onCopy }) {
  const { subtotal, discountAmt, total, partialPayment, remaining } = calcTotals(doc);
  const isInv  = doc.type === "invoice";
  const isEst  = doc.type === "estimate";
  ...
  const docNo  = doc.docNumber || doc.reportNumber || "";
  const typeName = isInv ? "請求書" : isEst ? "見積書" : doc.type === "report_tax" ? "報告書（税有）" : "報告書（税無）";
  const canCopy = isOriginalCopy(docNo); // 枝番01のみコピー可
      ...
      <div style={{ background: "#fff", borderRadius: 10, width: "100%", maxWidth: 760, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 48px #0005", fontFamily: "inherit" }}>
        ...
        <div style={{ background: accent, color: gold, padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", borderRadius: "10px 10px 0 0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: "1.1rem", fontWeight: 700, letterSpacing: "0.1em" }}>{typeName}</span>
            <span style={{ fontSize: "0.8rem", opacity: 0.8 }}>{docNo}</span>
          </div>
        ...
        <div style={{ padding: "24px" }}>
          ...
          {/* 報告書の明細（reportItems） */}
          {isReport && (
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.78rem", marginBottom: 14 }}>
              <thead><tr>
                {[["施行日","14%","left"],["ご使用者","18%","left"],["ご紹介内容","26%","left"],["率","8%","center"],["ご請求金額","17%","right"],["リベート額","17%","right"]].map(([h,w,a]) => (
                  <th key={h} style={{ background: accent, color: gold, padding: "6px 8px", textAlign: a, width: w, fontWeight: 600 }}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {(doc.reportItems || []).filter((r) => r.user || r.referral || r.chargeAmount).map((r, i) => {
                  const rebate = Math.round(Number(r.chargeAmount || 0) * (Number(r.rebateRate || 0) / 100));
                  return (
                    <tr key={i} style={{ background: i % 2 === 0 ? "#fffdf9" : "#f7f3ee" }}>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #e0d8cc" }}>{r.workDate}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #e0d8cc" }}>{r.user}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #e0d8cc" }}>{r.referral}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #e0d8cc", textAlign: "center" }}>{r.rebateRate}%</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #e0d8cc", textAlign: "right" }}>¥ {fmt(r.chargeAmount)}</td>
                      <td style={{ padding: "5px 8px", borderBottom: "1px solid #e0d8cc", textAlign: "right", fontWeight: 600 }}>¥ {fmt(rebate)}</td>
                    </tr>
                  );
                })}
              </tbody>
...
export default function App() {
  ...
  useEffect(() => {
    loadFirebase().then(db => {
      ...
      // docs・trash はリアルタイム購読（購読エラー時は offline 表示に切替）
  ...
  const setItem       = (idx, patch) => setForm((f) => ({ ...f, items: f.items.map((it, i) => {
    if (i !== idx) return it;
    const merged = { ...it, ...patch };
    // patch で undefined を渡したキーは「削除」扱いにする（Firestore は undefined を保存できないため）
    for (const k in patch) { if (patch[k] === undefined) delete merged[k]; }
    return merged;
  }) }));
  ...
  const saveDoc = () => {
    ...
    // ローカル更新
    const nextDocs = existing ? docs.map((d) => d.id === newDoc.id ? newDoc : d) : [...docs, newDoc];
    setDocs((prev) => existing ? prev.map((d) => d.id === newDoc.id ? newDoc : d) : [...prev, newDoc]);
    setForm(newDoc);
    // Firebase に保存（接続中）。失敗時はFirebaseに保存できなかった旨を通知
    if (fbDb) {
      fbSaveDoc(fbDb, COLLECTIONS.docs, newDoc).catch((err) => {
        console.error("Firebase保存エラー:", err);
        setFbStatus("offline");
        alert("⚠️ クラウドへの保存に失敗しました。\nこの書類は端末内には保存されていますが、他端末に共有されていない可能性があります。\nネット接続を確認し、再度保存してください。");
      });
    }
    // オンライン・オフラインに関わらず、端末内にも必ずバックアップ保存
    persist(KEYS.docs, nextDocs);
    setSaveMsg(true);
    setTimeout(() => setSaveMsg(false), 2500);
  };

  // 既存書類（枝番01）をコピーして、枝番02,03…の新しい書類を作る
  const copyDoc = (src) => {
    const srcNo  = getDocNo(src);
    const newNo  = formatCopyNumber(srcNo, docs);
    if (!newNo) {
      alert("この書類は旧い番号形式のため、コピーできません。\n新しい番号ルールで作成した書類（01）からコピーしてください。");
      return;
    }
    const isReport = src.type === "report_tax" || src.type === "report_notax";
    // 中身を引き継ぎつつ、ID・番号・編集ログだけ新しくする
    const newDoc = {
      ...src,
      id:        genId(),
      docNumber:    isReport ? "" : newNo,
      reportNumber: isReport ? newNo : "",
      editLog:   [{ at: nowStr(), changes: [`コピー作成（元: ${srcNo}）`] }],
    };
    delete newDoc._fbId; // Firestore購読由来の内部IDは引き継がない
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 48, borderBottom: "1px solid #c8a96e22" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", lineHeight: 1 }}>
              ...
              <span style={{ fontSize: "0.62rem", color: "#c8a96e88", letterSpacing: "0.1em", marginTop: 1 }}>Ver1-015</span>
            </div>
